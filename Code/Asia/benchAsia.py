...
Original file is located at
    https://colab.research.google.com/drive/1Z-TQqpNH0Oi6uTTjHTW79ynYezEtg-TT
"""
...
# GES
ges_pgmpy_est = pgmpy.estimators.GES(data=asia_data)
ges_pgmpy = ges_pgmpy_est.estimate()

# Hill Climb Search
hcs_pgmpy_est = HillClimbSearch(data=asia_data)
hcs_pgmpy = hcs_pgmpy_est.estimate()

# Tree Search
ts_est = TreeSearch(asia_data)
ts_cl_pgmpy = ts_est.estimate(estimator_type="chow-liu")
# ts_est = TreeSearch(asia_data)
# ts_tan_pgmpy = ts_est.estimate(estimator_type="tan")         # Requires a class node

#Expert in the Loop
# eitl_pgmpy = ExpertInLoop(asia_data).estimate(
#     effect_size_threshold=0.0001, orientation_fn=manual_pairwise_orient
# )

#Mhmc
mhmc_est = MmhcEstimator(asia_data)
mhmc_pgmpy = mhmc_est.estimate()

# #Exhaustive Search
# es_est = ExhaustiveSearch(data=asia_data, scoring_method=K2(asia_data))
# es_pgmpy = es_est.estimate()
...
ges_pgmpy_viz = ges_pgmpy.to_graphviz()
ges_pgmpy_viz.draw('ges_pgmpy.png', prog='neato')
Image('ges_pgmpy.png')

hcs_pgmpy_viz = hcs_pgmpy.to_graphviz()
hcs_pgmpy_viz.draw('hcs_pgmpy.png', prog='neato')
Image('hcs_pgmpy.png')

ts_pgmpy_viz = ts_cl_pgmpy.to_graphviz()
ts_pgmpy_viz.draw('ts_pgmpy.png', prog='neato')
Image('ts_pgmpy.png')

mhmc_pgmpy_viz = mhmc_pgmpy.to_graphviz()
mhmc_pgmpy_viz.draw('mhmc_pgmpy.png', prog='neato')
Image('mhmc_pgmpy.png')
...
def get_f1_score(estimated_model, true_model,mname):
    ...
    print("F1-score for the model skeleton" , str(mname),": ", f1)

get_f1_score(pc_pgmpy, asia_model, "PC")
get_f1_score(ges_pgmpy, asia_model, "GES")
get_f1_score(hcs_pgmpy, asia_model, "Hill Climb Search")
get_f1_score(ts_cl_pgmpy, asia_model, "Tree Search Chow Liu")
# get_f1_score(ts_tan_pgmpy, asia_model, "Tree Search Tan")
# get_f1_score(eitl_pgmpy, asia_model, "Expert in the Loop")
get_f1_score(mhmc_pgmpy, asia_model, "Mhmc")
# get_f1_score(es_pgmpy, asia_model, "Exhaustive Search")
